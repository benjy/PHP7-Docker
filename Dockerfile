...
RUN sudo apt-get install autoconf bison libxml2-dev apache2-dev libfreetype6-dev libmcrypt-dev openssl libcurl4-openssl-dev -y

# build the ./configure script
RUN cd /src && ./buildconf

# Configure with an apache shared .so file.
RUN cd /src && ./configure --with-apxs2=/usr/bin/apxs --enable-maintainer-zts --with-pdo-mysql --with-gd --enable-mbstring --enable-exif --with-mcrypt --with-openssl=/usr --with-libdir=/lib/x86_64-linux-gnu/
